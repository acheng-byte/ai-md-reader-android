/* MD 阅读器 iOS 前端 v1.9.4
   ...
   表格/图表预览、下载、脚注、Callout、Wikilink 等全部功能。 */
...
if (typeof window.Android === 'undefined') {
    window.Android = {
        ...
        getSettingsJson: function () { return JSON.stringify(_iosSettings); },
        getInitialMode: function () { return _iosMode; },
        getInitialScrollRatio: function () { return _iosScrollRatio; },
        getTitle: function () { return _iosTitle; },
        copyText: function (t) { _iosPost({ type: 'copy', text: t }); },
        onCenterTap: function () { _iosPost({ type: 'centerTap' }); },
        ...
        searchVaultForEmbed: function (ref) { return null; },
        loadEmbedContent: function (uri) { return null; },
        openVaultFile: function (uri) { _iosPost({ type: 'openVault', uri: uri }); },
        openWikiLink: function (name) { _iosPost({ type: 'openWiki', name: name }); }
    };
}
...
(function () {
    ...
    // iOS: 保留已有的桥接包装函数，内部转发到 applySettings/setMode
    // Android: 直接赋值
    if (!window._iosBridgeActive) {
        window.appApplySettings = applySettings;
        window.appSetMode = setMode;
    } else {
        // iOS 桥接已激活：原生调用 appApplySettings → 更新全局 → 调用内部 applySettings
        window._iosApplySettingsNow = applySettings;
    }
    ...
    try {
        ...
        if (b0 && !isIos) {
            ...
            // iOS 初始化：使用全局变量中的设置（可能为空），等原生 push 内容和设置
            initMermaid(false);
            if (_iosSettings && _iosSettings.dark != null) {
                initMermaid(!!_iosSettings.dark);
            }
        }

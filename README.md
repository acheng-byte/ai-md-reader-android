...
一款为 AI 时代准备的轻量级手机 Markdown 阅读器。Android / 支持 APK 安装的鸿蒙设备已更新到 v1.6.2，iOS v1.3 源码已加入并完成模拟器构建 / 安装 / 启动冒烟测试。
...
v1.6.2 集中修复了 Obsidian 兼容性问题并增强渲染能力：

- **修复**：`[[文件夹/文件名]]` 路径式 Wikilink 点击提示”找不到文件”——现已支持多级路径导航
- **修复**：目录面板中标题前多余的 `#` 符号已去除，点击目录项不再无响应
- **修复**：TXT 文件（GBK / GB2312 编码）打开乱码，现自动检测编码
- **新增**：启动时自动弹出打开历史面板，快速继续上次阅读
- **新增**：`==高亮==` 黄色背景高亮（Obsidian 语法）
- **新增**：`#标签` 样式化标签显示
- **新增**：`%%注释%%` 注释内容自动隐藏
- **新增**：脚注 `[^1]` 支持，文末自动生成脚注列表
- **新增**：`[[#标题]]` 内部锚点链接

[下载 Android v1.6.2 APK](https://github.com/acheng-byte/ai-md-reader-android/releases/download/v1.6.2/MDReader-1.6.2.apk) · [查看 v1.6.2 Release](https://github.com/acheng-byte/ai-md-reader-android/releases/tag/v1.6.2) · iOS 版目前提供源码，TestFlight 待 Apple Developer 签名账号接入。
...
| 版本 | 类型 | 更新重点 |
| --- | --- | --- |
| [v1.6.2](https://github.com/acheng-byte/ai-md-reader-android/releases/tag/v1.6.2) | 最新版本 | 修复路径式 Wikilink、目录 # 前缀、TOC 点击、TXT 乱码；新增高亮/标签/注释/脚注等 Obsidian 语法；启动自动弹出历史 |
| [v1.6.0](https://github.com/acheng-byte/ai-md-reader-android/releases/tag/v1.6.0) | 历史记录 | 支持 .txt/.docx/.doc；Vault 图片视频加载；![[doc.md]] 内联展开；Frontmatter/引用样式开关；全库异步搜索；自动更新 |
| [v1.4.1](https://github.com/acheng-byte/ai-md-reader-android/releases/tag/v1.4.1) | 历史记录 | 新增阅读位置记忆；修复已收藏文档在打开历史中误报”授权过期”的问题 |

...
一款为 AI 时代准备的轻量级手机 Markdown 阅读器。Android / 支持 APK 安装的鸿蒙设备已更新到 v1.4，iOS v1.3 源码已加入并完成模拟器构建 / 安装 / 启动冒烟测试。
...
v1.4 重点补上“读完之后顺手发给别人”的动作，让 AI 生成的方案、笔记、纪要和代码说明不再卡在手机里：

- **一键转发完整文件**：阅读时点击工具栏“转发”，把当前 `.md` / `.markdown` 文档作为文件交给系统分享面板。
- **更适合微信等社交场景**：可转发到微信、QQ、邮件、文件应用等，接收方拿到的是可再次打开的 Markdown 文件，而不是容易丢格式的纯文本。
- **轻量安全**：通过 FileProvider 临时授权共享缓存文件，不申请额外存储权限，也不把文档上传到服务端。
- **Android / 鸿蒙兼容设备更新**：本次 v1.4 为 Android APK 侧更新；支持 APK 安装的鸿蒙设备可按同样方式试用。
- **iOS 仍停留在 v1.3 源码**：iOS 版暂未加入转发功能，TestFlight / 真机发布仍需 Apple Developer 签名配置。

[下载 Android v1.4 APK](https://github.com/mizzlelover/ai-md-reader-android/releases/download/v1.4/MDReader-1.4.apk) · [查看 v1.4 Release](https://github.com/mizzlelover/ai-md-reader-android/releases/tag/v1.4) · iOS 版目前提供源码，TestFlight 待 Apple Developer 签名账号接入。

## 平台状态

| 平台 | 当前状态 |
| --- | --- |
| Android | v1.4 已发布 APK，可在 GitHub Releases 下载 |
...
| 版本 | 类型 | 更新重点 |
| --- | --- | --- |
| [v1.4](https://github.com/mizzlelover/ai-md-reader-android/releases/tag/v1.4) | 最新版本 | 新增「转发」：阅读时一键把完整 `.md` 文档经系统分享转发到微信等应用（如微信好友）；本次为 Android / 鸿蒙兼容设备更新 |
...
8. 在微信收到 `.md` 文件时，选择“用其他应用打开”，再选择“MD阅读器”。
